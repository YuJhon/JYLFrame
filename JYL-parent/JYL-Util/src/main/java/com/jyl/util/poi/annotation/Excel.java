...
public @interface Excel {
    ...
    /**
     * 导出时在excel中每个列的宽 单位为字符，一个汉字=2个字符 如 以列名列内容中较合适的长度 例如姓名列6 【姓名一般三个字】 性别列4【男女占1，但是列标题两个汉字】
     * 限制1-255
     */
    ...
    /**
     * 若是sign为1,则需要在pojo中加入一个方法 convertGet字段名() 例如，字段sex ，需要加入 public String convertGet字段名()
     * 返回值为string 若是sign为0,则不转换 建议使用 @Transient 这个注解让hibernate忽略这个方法
     */
